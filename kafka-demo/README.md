...
以上任意环节出现问题, 都会导致上游Kafka积压

1. 读数据: 很难称为瓶颈
2. 数据计算: 可能出现时间复杂度过高的情况, 或者出现网络IO, 可能出现瓶颈
3. 结果输出: 一般写入数据库, 出入数据时,数据库会进行排序索引等操作, 随着数据量越来越大, 导致数据写入缓慢

+ 增加鞋服的buffer大小
+ 多次频繁写入->攒批写入
+ 增加写入端并行度 / 单机表->分布式表

以上都没问题的情况下, 因为意外停机导致的数据积压, 需要重新恢复时: 提高消费端并行度, 增加partition数量